...
using pyimage_t = Eigen::Matrix<dtype, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>;

typedef Eigen::Matrix<float, Eigen::Dynamic, ddim, Eigen::RowMajor> descriptors_t;
...
public:
        ...
        if (use_gpu_) {
            ...
            if (sift_gpu_mutexes.count(sift_gpu->gpu_index) == 0) {
                sift_gpu_mutexes.emplace(
                    sift_gpu->gpu_index, std::unique_ptr<std::mutex>(new std::mutex()));
            }
            #endif
        }
    }
    ...
    template<typename dtype>
    sift_output_t Extract(Eigen::Ref<const pyimage_t<dtype>> image, bool do_normalize) {
...
private:
    ...
    sift_output_t ExtractGPU(const pyimage_t<dtype>& image /* [h, w] */) {
        ...
        if (std::is_same<dtype, float>::value) {
            code =
            sift_gpu->RunSIFT(image.cols(), image.rows(),
                              image.data(), GL_LUMINANCE, GL_FLOAT);
        } else if (std::is_same<dtype, uint8_t>::value) {
            code =
            sift_gpu->RunSIFT(image.cols(), image.rows(),
                              image.data(), GL_LUMINANCE, GL_UNSIGNED_BYTE);
        } else {
            THROW_EXCEPTION(std::runtime_error,"SIFT GPU only support float/byte images.");
        }
    ...
    sift_output_t ExtractCPU(
        Eigen::Ref<const pyimage_t<float>> image
    )  {
        // Create a new instance of SIFT detector & descriptor.
        VlSiftFilt *sift = vl_sift_new(image.cols(), image.rows(),
        ...
        while (true) {
            ...
            for (int i = 0; i < num_keypoints; ++i) {
                ...
                for (int o = 0; o < num_orientations; ++o) {
                    ...
                    vl_sift_calc_keypoint_descriptor(sift, descriptors.row(num_feats).data(), &vl_keypoints[i], angles[o]);
                    // Recover score.
                    scores(num_feats) = dog[vl_keypoints[i].is * octave_width * octave_height +
                                            vl_keypoints[i].iy * octave_width + vl_keypoints[i].ix];
                    num_feats++;
                }
            }
        }
    ...
    sift_output_t ExtractCPU(
        Eigen::Ref<const pyimage_t<uint8_t>> image
    ) {
