...
class Sift {
  ...
  Sift(SiftExtractionOptions options, Device device)
    ...
    if (use_gpu_) {
      ...
      if (sift_gpu_mutexes.count(sift_gpu->gpu_index) == 0) {
        sift_gpu_mutexes.emplace(sift_gpu->gpu_index,
 ...
 private:
  ...
  sift_output_t ExtractGPU(const pyimage_t<dtype>& image /* [h, w] */) {
    ...
    std::unique_lock<std::mutex> lock(*sift_gpu_mutexes[sift_gpu->gpu_index]);
    ...
    if (std::is_same<dtype, float>::value) {
      code = sift_gpu->RunSIFT(image.cols(), image.rows(), image.data(),
  ...
  sift_output_t ExtractCPU(Eigen::Ref<const pyimage_t<float>> image) {
    ...
    VlSiftFilt* sift =
        vl_sift_new(image.cols(), image.rows(), options_.num_octaves,
    ...
    while (true) {
      ...
      for (int i = 0; i < num_keypoints; ++i) {
        ...
        for (int o = 0; o < num_orientations; ++o) {
          ...
          vl_sift_calc_keypoint_descriptor(sift,
...
sift_output_t extract_sift(const py::array_t<float> image,
                           const int num_octaves, const int octave_resolution,
                           const int first_octave, const float edge_thresh,
                           const float peak_thresh, const bool upright) {
...
void init_sift(py::module& m) {
  m.def("extract_sift", &extract_sift, py::arg("image"),
        py::arg("num_octaves") = 4, py::arg("octave_resolution") = 3,
        py::arg("first_octave") = 0, py::arg("edge_thresh") = 10.0,
        py::arg("peak_thresh") = 0.01, py::arg("upright") = false,
  ...
  py::class_<Sift>(m, "Sift")
      .def(py::init<SiftExtractionOptions, Device>(),
           py::arg("options") = sift_options, py::arg("device") = Device::AUTO)
      .def("extract", &Sift::Extract<float>, py::arg("image"),
           py::arg("do_normalize") = false)
      .def("extract", &Sift::Extract<uint8_t>, py::arg("image").noconvert(),
           py::arg("do_normalize") = false)
