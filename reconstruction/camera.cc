...
void init_camera(py::module& m) {
  ...
  py::class_<Camera, std::shared_ptr<Camera>>(m, "Camera")
      ...
      .def(py::init([](const std::string& name, size_t width, size_t height,
           ...
           py::arg("model"), py::arg("width"), py::arg("height"),
           py::arg("params"), py::arg("id") = kInvalidCameraId)
      ...
      .def_property("focal_length", &Camera::FocalLength,
                    &Camera::SetFocalLength)
      .def_property("focal_length_x", &Camera::FocalLengthX,
                    &Camera::SetFocalLengthX)
      .def_property("focal_length_y", &Camera::FocalLengthY,
                    &Camera::SetFocalLengthY)
      .def_property("has_prior_focal_length", &Camera::HasPriorFocalLength,
                    &Camera::SetPriorFocalLength)
      .def_property("principal_point_x", &Camera::PrincipalPointX,
                    &Camera::SetPrincipalPointX)
      .def_property("principal_point_y", &Camera::PrincipalPointY,
      ...
      .def_property(
          ...
          &Camera::SetParams, "Camera parameters.")
      ...
      .def(
          "initialize_with_id",
          [](Camera& self, const int model_id, const double focal_length,
      ...
      .def(
          ...
          [](Camera& self, std::string model_name, const double focal_length,
             const size_t width, const size_t height) {
            THROW_CHECK(ExistsCameraModelWithName(model_name));
      ...
      .def(
          ...
          [](const Camera& self, const std::vector<Point2D>& world_points2D) {
            std::vector<Eigen::Vector2d> image_points2D;
            for (size_t idx = 0; idx < world_points2D.size(); ++idx) {
              image_points2D.push_back(
                  self.WorldToImage(world_points2D[idx].XY()));
            }
            return image_points2D;
          },
          "Project list of points from world / infinity to image plane.")
