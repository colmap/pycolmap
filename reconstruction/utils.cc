...
inline bool ComputeRobustAlignmentBetweenPoints(
...
inline SimilarityTransform3 AlignPosesBetweenReconstructions(
    Reconstruction& self, const Reconstruction& ref,
  ...
  bool success = ComputeAlignmentBetweenReconstructions(
      self, ref, min_inlier_observations, max_reproj_error, &alignment);
  THROW_CHECK(success);
...
inline SimilarityTransform3 AlignPointsBetweenReconstructions(
    Reconstruction& self, const Reconstruction& ref, int min_overlap,
...
inline SimilarityTransform3 AlignReconstruction(
...
inline SimilarityTransform3 RobustAlignReconstruction(
    Reconstruction& self, const std::vector<std::string>& image_names,
  ...
  bool success = self.AlignRobust(image_names, locations, min_common_images,
                                  options, &tform);
  THROW_CHECK(success);
  return tform;
}
...
void init_reconstruction_utils(py::module& m) {
  m.def(
      ...
      [](const Reconstruction& reconstruction1,
        ...
        THROW_CHECK_GE(min_inlier_observations, 0.0);
        THROW_CHECK_LE(min_inlier_observations, 1.0);
        auto PrintComparisonSummary =
            [](std::stringstream& ss, std::vector<double>& rotation_errors,
              ...
              auto PrintErrorStats = [](std::stringstream& ss,
                ...
                if (len == 0) {
                  ss << "Cannot extract error statistics from empty input"
                     << std::endl;
                  return;
                }
        ...
        Eigen::Matrix3x4d alignment;
        if (!ComputeAlignmentBetweenReconstructions(
                reconstruction2, reconstruction1, min_inlier_observations,
        ...
        for (size_t i = 0; i < num_images; ++i) {
          ...
          const Eigen::Vector4d normalized_qvec1 =
              NormalizeQuaternion(image1.Qvec());
          const Eigen::Quaterniond quat1(
              normalized_qvec1(0), normalized_qvec1(1), normalized_qvec1(2),
              normalized_qvec1(3));
          const Eigen::Vector4d normalized_qvec2 =
              NormalizeQuaternion(image2.Qvec());
          const Eigen::Quaterniond quat2(
              normalized_qvec2(0), normalized_qvec2(1), normalized_qvec2(2),
              normalized_qvec2(3));
        ...
        py::dict res("alignment"_a = alignment,
      ...
      py::arg("max_reproj_error") = 8.0, py::arg("verbose") = true,
