...
void init_image(py::module& m) {
  ...
  py::class_<colmap::Image, std::shared_ptr<colmap::Image>>(m, "Image")
      ...
      .def(py::init([](const std::string& name,
                       ...
                       size_t camera_id, colmap::image_t image_id) {
      ...
      .def_property("qvec", overload_cast_<>()(&Image::Qvec), &Image::SetQvec,
                    "Quaternion vector (qw,qx,qy,qz) which describes rotation "
                    "from world to image space.")
      .def_property("tvec", overload_cast_<>()(&Image::Tvec), &Image::SetTvec,
                    "Translation vector (tx,ty,tz) which describes translation "
                    "from world to image space.")
      .def_property("qvec_prior", overload_cast_<>()(&Image::QvecPrior),
                    &Image::SetQvecPrior,
                    "Quaternion prior, e.g. given by EXIF gyroscope tag.")
      .def_property("tvec_prior", overload_cast_<>()(&Image::TvecPrior),
                    &Image::SetTvecPrior,
      ...
      .def(
          ...
          [](Image& self, const point2D_t point2D_idx,
      ...
      .def_property(
          "num_observations", &Image::NumObservations,
      ...
      .def("get_valid_point2D_ids",
           [](const Image& self) {
             ...
             for (colmap::point2D_t point2D_idx = 0;
                  point2D_idx < self.NumPoints2D(); ++point2D_idx) {
           ...
           [](const Image& self, const Reconstruction& rec) {
             ...
             for (auto& p2D : self.Points2D()) {
               ...
               world_points.push_back(
                   (projection_matrix *
                    rec.Point3D(p2D.Point3DId()).XYZ().homogeneous())
                       .hnormalized());
             }
             return world_points;
           })
      ...
      .def(
          ...
          [](const Image& self,
            ...
            for (int idx = 0; idx < world_coords.size(); ++idx) {
              image_points[idx] =
                  (projection_matrix * world_coords[idx].homogeneous())
                      .hnormalized();
            }
            return image_points;
          },
      ...
      .def(
          ...
          [](const Image& self, const std::vector<Point3D>& point3Ds) {
            ...
            for (int idx = 0; idx < point3Ds.size(); ++idx) {
              world_points[idx] =
                  (projection_matrix * point3Ds[idx].XYZ().homogeneous())
                      .hnormalized();
            }
            return world_points;
          },
      ...
      .def(
          ...
          [](const Image& self,
            ...
            const Eigen::Matrix3x4d inv_projection_matrix =
                self.InverseProjectionMatrix();
            std::vector<Eigen::Vector3d> world_points(image_coords.size());
            for (int idx = 0; idx < image_coords.size(); ++idx) {
              world_points[idx] =
                  (inv_projection_matrix * image_coords[idx].homogeneous());
            }
            return world_points;
          },
