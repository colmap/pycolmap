...
py::dict world_to_image(const std::vector<Eigen::Vector2d> world_points2D,
                        const colmap::Camera& camera) {
  static unsigned int m_warnings = 0;
  m_warnings++;
  if (m_warnings == 1) {
...
void init_transforms(py::module& m) {
  m.def(
      ...
      [](const Reconstruction& src_reconstruction,
        ...
        bool success = ComputeAlignmentBetweenReconstructions(
            src_reconstruction, ref_reconstruction, min_inlier_observations,
      ...
      py::arg("max_reproj_error") = 8.0, py::keep_alive<1, 2>(),
      py::keep_alive<1, 3>());
  ...
  py::class_<SimilarityTransform3>(m, "SimilarityTransform3")
      ...
      .def("transform_point",
           [](const SimilarityTransform3& self,
      ...
      .def(
          "transform_pose",
          [](const SimilarityTransform3& self, Eigen::Ref<Eigen::Vector4d> qvec,
  ...
  m.def("qvec_to_rotmat", &colmap::QuaternionToRotationMatrix, py::arg("qvec"),
        "Convert COLMAP quaternion to rotation matrix");
  m.def("rotmat_to_qvec", &colmap::RotationMatrixToQuaternion,
        py::arg("rotmat"), "Convert rotation matrix to colmap quaternion");
  m.def("qvec_rotate_point", &colmap::QuaternionRotatePoint, py::arg("qvec"),
        py::arg("xyz"), "Rotate world point");
  m.def("concat_quat", &colmap::ConcatenateQuaternions, py::arg("qvec1"),
        py::arg("qvec2"),
        "Concatenate Quaternion rotations such that the rotation of qvec1 is "
        "applied"
        "before the rotation of qvec2.");
  m.def("invert_qvec", &colmap::InvertQuaternion, py::arg("qvec"),
        "Returns inverted qvec");
  m.def("normalize_qvec", &colmap::InvertQuaternion, py::arg("qvec"),
  ...
  m.def(
      "relative_pose",
      [](const Eigen::Vector4d& qvec1, const Eigen::Vector3d& tvec1,
         const Eigen::Vector4d& qvec2, const Eigen::Vector3d& tvec2) {
        Eigen::Vector4d qvec12;
        Eigen::Vector3d tvec12;
        colmap::ComputeRelativePose(qvec1, tvec1, qvec2, tvec2, &qvec12,
                                    &tvec12);
  ...
  m.def(
      "world_to_image",
      [](int model_id, Eigen::Ref<Eigen::VectorXd> camera_params,
         Eigen::Ref<Eigen::Vector2d> uv, Eigen::Ref<Eigen::Vector2d> xy) {
        int num_params;
        switch (model_id) {
#define CAMERA_MODEL_CASE(CameraModel)                                    \
  case CameraModel::kModelId:                                             \
    ...
    CameraModel::WorldToImage<double>(camera_params.data(), uv(0), uv(1), \
                                      xy.data(), xy.data() + 1);          \
    break;
...
#undef CAMERA_MODEL_CASE
      ...
      py::arg("model_id"), py::arg("camera_params").noconvert(),
      py::arg("uv").noconvert(), py::arg("xy").noconvert());
  ...
  m.def(
      "image_to_world",
      [](int model_id, Eigen::Ref<Eigen::VectorXd> camera_params,
         Eigen::Ref<Eigen::Vector2d> xy, Eigen::Ref<Eigen::Vector2d> uv) {
        int num_params;
        switch (model_id) {
#define CAMERA_MODEL_CASE(CameraModel)                                    \
  case CameraModel::kModelId:                                             \
    num_params = CameraModel::kNumParams;                                 \
    THROW_CHECK_EQ(num_params, camera_params.size());                     \
    CameraModel::ImageToWorld<double>(camera_params.data(), xy(0), xy(1), \
                                      uv.data(), uv.data() + 1);          \
    break;
          CAMERA_MODEL_SWITCH_CASES
#undef CAMERA_MODEL_CASE
      ...
      py::arg("model_id"), py::arg("camera_params").noconvert(),
      py::arg("xy").noconvert(), py::arg("uv").noconvert());
}
