...
py::dict absolute_pose_estimation(
    ...
    const std::vector<Eigen::Vector3d> points3D, Camera& camera,
  ...
  if (!EstimateAbsolutePose(estimation_options, points2D, points3D, &qvec,
                            &tvec, &camera, &num_inliers, &inlier_mask)) {
    return failure_dict;
  }

  // Absolute pose refinement.
  Eigen::Matrix<double, 6, 6> covariance;
  if (!RefineAbsolutePose(refinement_options, inlier_mask, points2D, points3D,
                          &qvec, &tvec, &camera,
...
py::dict absolute_pose_estimation(const std::vector<Eigen::Vector2d> points2D,
                                  ...
                                  Camera& camera, const double max_error_px,
  ...
  return absolute_pose_estimation(points2D, points3D, camera, abs_pose_options,
                                  abs_pose_refinement_options,
                                  return_covariance);
}

py::dict pose_refinement(
    const Eigen::Vector3d tvec, const Eigen::Vector4d qvec,
    const std::vector<Eigen::Vector2d> points2D,
    const std::vector<Eigen::Vector3d> points3D,
    const std::vector<bool> inlier_mask, const Camera camera,
    const AbsolutePoseRefinementOptions refinement_options) {
  SetPRNGSeed(0);
  ...
  // Absolute pose refinement.
  if (!RefineAbsolutePose(refinement_options, inlier_mask_char, points2D,
                          points3D, &qvec_refined, &tvec_refined,
...
void bind_absolute_pose_estimation(py::module& m,
  ...
  m.def("absolute_pose_estimation",
        static_cast<py::dict (*)(const std::vector<Eigen::Vector2d>,
                                 const std::vector<Eigen::Vector3d>, Camera&,
  ...
  m.def("pose_refinement", &pose_refinement, py::arg("tvec"), py::arg("qvec"),
        py::arg("points2D"), py::arg("points3D"), py::arg("inlier_mask"),
        py::arg("camera"), py::arg("refinement_options") = ref_options,
