...
py::dict two_view_geometry_estimation(
    ...
    const std::vector<Eigen::Vector2d> points2D2, Camera& camera1,
    Camera& camera2, TwoViewGeometry::Options options) {
  SetPRNGSeed(0);
  ...
  two_view_geometry.EstimateCalibrated(camera1, points2D1, camera2, points2D2,
                                       matches, options);

  if (!two_view_geometry.EstimateRelativePose(camera1, points2D1, camera2,
                                              points2D2)) {
    return failure_dict;
  }
  ...
  // See
  // https://github.com/colmap/colmap/blob/dev/src/estimators/two_view_geometry.h#L48
...
py::dict two_view_geometry_estimation(
    const std::vector<Eigen::Vector2d> points2D1,
    const std::vector<Eigen::Vector2d> points2D2, Camera& camera1,
    Camera& camera2, const double max_error_px, const double min_inlier_ratio,
    const int min_num_trials, const int max_num_trials,
  ...
  return two_view_geometry_estimation(points2D1, points2D2, camera1, camera2,
                                      two_view_geometry_options);
}

void bind_two_view_geometry_estimation(
    py::module& m, py::class_<RANSACOptions> PyRANSACOptions) {
  auto PyEstimationOptions =
      py::class_<TwoViewGeometry::Options>(m, "TwoViewGeometryOptions")
          ...
          .def_readwrite("min_num_inliers",
                         &TwoViewGeometry::Options::min_num_inliers)
          .def_readwrite("min_E_F_inlier_ratio",
                         &TwoViewGeometry::Options::min_E_F_inlier_ratio)
          .def_readwrite("max_H_inlier_ratio",
                         &TwoViewGeometry::Options::max_H_inlier_ratio)
          .def_readwrite("watermark_min_inlier_ratio",
                         &TwoViewGeometry::Options::watermark_min_inlier_ratio)
          .def_readwrite("watermark_border_size",
                         &TwoViewGeometry::Options::watermark_border_size)
          .def_readwrite("detect_watermark",
                         &TwoViewGeometry::Options::detect_watermark)
          .def_readwrite("multiple_ignore_watermark",
                         &TwoViewGeometry::Options::multiple_ignore_watermark)
  ...
  m.def("two_view_geometry_estimation",
        static_cast<py::dict (*)(const std::vector<Eigen::Vector2d>,
                                 ...
                                 Camera&, const TwoViewGeometry::Options)>(
            &two_view_geometry_estimation),
        py::arg("points2D1"), py::arg("points2D2"), py::arg("camera1"),
        py::arg("camera2"), py::arg("estimation_options") = est_options,
  ...
  m.def(
      ...
      static_cast<py::dict (*)(const std::vector<Eigen::Vector2d>,
                               const std::vector<Eigen::Vector2d>, Camera&,
                               Camera&, const double, const double, const int,
