...
py::dict rig_absolute_pose_estimation(
  ...
  for (size_t i = 0; i < points2D.size(); ++i) {
    for (size_t j = 0; j < points2D[i].size(); ++j) {
      ...
      points2D_rig.back().ray_in_cam =
          cameras[i].CamFromImg(points2D[i][j]).homogeneous().normalized();
      points2D_rig.back().cam_from_rig = cams_from_rig[i];
      camera_idxs.push_back(i);
    }
    points3D_all.insert(
        points3D_all.end(), points3D[i].begin(), points3D[i].end());
    points2D_all.insert(
        points2D_all.end(), points2D[i].begin(), points2D[i].end());

    error_threshold +=
        cameras[i].CamFromImgThreshold(max_error_px) * points2D[i].size();
  }
...
py::dict rig_absolute_pose_estimation(
  ...
  return rig_absolute_pose_estimation(points2D,
                                      points3D,
                                      cameras,
                                      cams_from_rig,
                                      ransac_options,
...
void bind_generalized_absolute_pose_estimation(py::module& m) {
  ...
  m.def(
      ...
      static_cast<py::dict (*)(const std::vector<std::vector<Eigen::Vector2d>>,
                               ...
                               bool)>(&rig_absolute_pose_estimation),
      "points2D"_a,
      "points3D"_a,
      "cameras"_a,
      "cams_from_rig"_a,
  ...
  m.def(
      ...
      static_cast<py::dict (*)(const std::vector<std::vector<Eigen::Vector2d>>,
                               ...
                               const int,
                               const double,
                               const bool)>(&rig_absolute_pose_estimation),
      "points2D"_a,
      "points3D"_a,
      "cameras"_a,
      "cams_from_rig"_a,
