...
py::dict rig_absolute_pose_estimation(
    ...
    const std::vector<Eigen::Vector3d> rig_tvecs, RANSACOptions ransac_options,
  ...
  for (size_t i = 0; i < points2D.size(); ++i) {
    const Eigen::Matrix3x4d rig_tform =
        ComposeProjectionMatrix(rig_qvecs[i], rig_tvecs[i]);
    ...
    points3D_all.insert(points3D_all.end(), points3D[i].begin(),
                        points3D[i].end());
    points2D_all.insert(points2D_all.end(), points2D[i].begin(),
                        points2D[i].end());

    error_threshold +=
        cameras[i].ImageToWorldThreshold(max_error_px) * points2D[i].size();
  }
  ...
  unique_points3D.erase(std::unique(unique_points3D.begin(),
                                    unique_points3D.end(), equalVector3d),
                        unique_points3D.end());
  std::vector<size_t> p3D_ids;
  for (auto p3D : points3D_all) {
    p3D_ids.push_back(std::lower_bound(unique_points3D.begin(),
                                       unique_points3D.end(), p3D,
                                       lowerVector3d) -
  ...
  if (!RefineGeneralizedAbsolutePose(
          refinement_options, report.inlier_mask, points2D_all, points3D_all,
          camera_idxs, rig_qvecs, rig_tvecs, &qvec, &tvec,
...
py::dict rig_absolute_pose_estimation(
    ...
    const std::vector<Camera> cameras,
    const std::vector<Eigen::Vector4d> rig_qvecs,
    const std::vector<Eigen::Vector3d> rig_tvecs, const double max_error_px,
    const double min_inlier_ratio, const int min_num_trials,
    const int max_num_trials, const double confidence,
    const bool return_covariance) {
  ...
  return rig_absolute_pose_estimation(points2D, points3D, cameras, rig_qvecs,
                                      rig_tvecs, ransac_options,
                                      refinement_options, return_covariance);
}
...
void bind_generalized_absolute_pose_estimation(py::module& m) {
  ...
  m.def("rig_absolute_pose_estimation",
        static_cast<py::dict (*)(
            const std::vector<std::vector<Eigen::Vector2d>>,
            const std::vector<std::vector<Eigen::Vector3d>>,
            const std::vector<Camera>, const std::vector<Eigen::Vector4d>,
            const std::vector<Eigen::Vector3d>, const RANSACOptions,
            const AbsolutePoseRefinementOptions, bool)>(
            &rig_absolute_pose_estimation),
        py::arg("points2D"), py::arg("points3D"), py::arg("cameras"),
        ...
        "Absolute pose estimation with non-linear refinement for a "
        "multi-camera rig.");
  ...
  m.def("rig_absolute_pose_estimation",
        static_cast<py::dict (*)(
            ...
            const std::vector<std::vector<Eigen::Vector3d>>,
            const std::vector<Camera>, const std::vector<Eigen::Vector4d>,
            const std::vector<Eigen::Vector3d>, const double, const double,
