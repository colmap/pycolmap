...
void import_images(const py::object database_path_,
                   const py::object image_path_, const CameraMode camera_mode,
  ...
  THROW_CUSTOM_CHECK_MSG(
      ExistsCameraModelWithName(options.camera_model), std::invalid_argument,
...
void import_images(const py::object database_path_,
  ...
  return import_images(database_path_, image_path_, camera_mode, image_list,
                       options);
}
...
Camera infer_camera_from_image(const py::object image_path_,
  ...
  Bitmap bitmap;
  THROW_CUSTOM_CHECK_MSG(
      bitmap.Read(image_path, false), std::invalid_argument,
  ...
  camera.InitializeWithId(camera.ModelId(), focal_length, bitmap.Width(),
                          bitmap.Height());
  THROW_CUSTOM_CHECK_MSG(
      camera.VerifyParams(), std::invalid_argument,
...
void undistort_images(py::object output_path_, py::object input_path_,
                      py::object image_path_,
                      std::vector<std::string> image_list,
                      std::string output_type, CopyType copy_type,
  ...
  if (output_type == "COLMAP") {
    undistorter.reset(new COLMAPUndistorter(
        ...
        num_patch_match_src_images, copy_type, image_ids));
...
void init_images(py::module& m) {
  ...
  auto PyImageReaderOptions =
      py::class_<IROpts>(m, "ImageReaderOptions")
          .def(py::init<>())
          .def_readwrite("camera_model", &IROpts::camera_model,
                         "Name of the camera model.")
          .def_readwrite("mask_path", &IROpts::mask_path,
          ...
          .def_readwrite(
              ...
              "height)`.")
          .def_readwrite(
              "camera_mask_path", &IROpts::camera_mask_path,
  ...
  m.def("import_images",
        static_cast<void (*)(const py::object, const py::object,
                             const CameraMode, const std::vector<std::string>,
        ...
        py::arg("options") = reader_options, "Import images into a database");

  m.def(
      "import_images",
      static_cast<void (*)(const py::object, const py::object, const CameraMode,
  ...
  m.def("infer_camera_from_image", &infer_camera_from_image,
        py::arg("image_path"), py::arg("options") = reader_options,
        "Guess the camera parameters from the EXIF metadata");

  m.def("undistort_images", &undistort_images, py::arg("output_path"),
        py::arg("input_path"), py::arg("image_path"),
        py::arg("image_list") = std::vector<std::string>(),
        ...
        py::arg("verbose") = false, "Undistort images");
}
