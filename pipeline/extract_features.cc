...
void VerifyCameraParams(const std::string& camera_model,
  ...
  THROW_CUSTOM_CHECK_MSG(
      ExistsCameraModelWithName(camera_model), std::invalid_argument,
...
void extract_features(
    const py::object database_path_, const py::object image_path_,
    const std::vector<std::string> image_list, const CameraMode camera_mode,
    const std::string camera_model, ImageReaderOptions reader_options,
    SiftExtractionOptions sift_options, const Device device, bool verbose) {
  ...
  VerifyCameraParams(reader_options.camera_model, reader_options.camera_params);
...
void init_extract_features(py::module& m) {
  ...
  auto PyNormalization =
      py::enum_<SEOpts::Normalization>(m, "Normalization")
          ...
          .value("L2", SEOpts::Normalization::L2,
  ...
  auto PySiftExtractionOptions =
      py::class_<SEOpts>(m, "SiftExtractionOptions")
          ...
          .def_readwrite(
              "max_image_size", &SEOpts::max_image_size,
          ...
          .def_readwrite(
              "domain_size_pooling", &SEOpts::domain_size_pooling,
  ...
  /* PIPELINE */
  m.def("extract_features", &extract_features, py::arg("database_path"),
        py::arg("image_path"),
