...
Reconstruction triangulate_points(Reconstruction reconstruction,
  ...
  RunPointTriangulatorImpl(reconstruction, database_path, image_path,
                           output_path, options, clear_points);
...
std::map<size_t, Reconstruction> incremental_mapping(
    ...
    const py::object output_path_, const IncrementalMapperOptions& options) {
  std::string database_path = py::str(database_path_).cast<std::string>();
  THROW_CHECK_FILE_EXISTS(database_path);
  std::string image_path = py::str(image_path_).cast<std::string>();
  THROW_CHECK_DIR_EXISTS(image_path);
...
std::map<size_t, Reconstruction> incremental_mapping(
    const py::object database_path_, const py::object image_path_,
    const py::object output_path_, const int num_threads,
    const int min_num_matches) {
  IncrementalMapperOptions options;
  options.num_threads = num_threads;
  options.min_num_matches = min_num_matches;
  return incremental_mapping(database_path_, image_path_, output_path_,
                             options);
}
...
void init_sfm(py::module& m) {
  ...
  m.def("triangulate_points", &triangulate_points, py::arg("reconstruction"),
        py::arg("database_path"), py::arg("image_path"), py::arg("output_path"),
        py::arg("clear_points") = true, py::arg("options") = mapper_options,
  ...
  m.def("incremental_mapping",
        static_cast<std::map<size_t, Reconstruction> (*)(
            const py::object, const py::object, const py::object, const int,
            const int)>(&incremental_mapping),
        py::arg("database_path"), py::arg("image_path"), py::arg("output_path"),
